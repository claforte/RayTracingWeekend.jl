...
@inline @fastmath function hit(ms::MovingSphere{T}, r::Ray{T}, tmin::T, tmax::T) where T
...
@inline function hit(hittables::HittableList, r::Ray{T}, tmin::T, tmax::T) where T
    ...
    best_rec = nothing # by default, no hit  # pre-declaring this as ::Union{HitRecord,Nothing} causes catastrophic decrease in performance, 10X allocations!
    @inbounds for i in eachindex(hittables)
		h = hittables[i]
...
# @inline @fastmath function hit(box::Aabb{T}, r::Ray{T}, tmin::T, tmax::T) where T
...
# 	return nothing # CLAFORTE HACK: this should return true, i.e. a hit without HitRecord!
# end

# @inline function hit(bvh::BvhNode, r::Ray{T}, tmin::T, tmax::T) where T
#     if !hit(bvh_node.box, r, tmin, tmax) return nothing end

# 	hit_left = hit(bvh.left, r, tmin, tmax) # HitRecord or false...
# 	hit_right = hit(bvh.right, r, tmin, hit_left !== nothing ? hit_left.t : tmax)

# 	if hit_right !== nothing
